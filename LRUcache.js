class LRUCache {
    ...
    put(key, value) {
        ...
        if (node) {
          ...
          this.checkCapacity();
          this.hash[key] = this.cache.add(key, value);
        }
    }

    size() {
      let counter = 0, currentNode
    }

    checkCapacity() {
      if (Object.keys(this.hash).length === this.limit) {
        let key = this.cache.removeTail().key;
...
class DoubleLinkedList {
    ...
    update(node) {
        if (node !== this.head) {
            let removedNode = this.removeNode(node);
            if (removedNode === this.tail) {
              this.tail = removedNode.prev;
            }
            this.setHead(removedNode);
        } else {
          return;
        }
    }
    ...
    removeNode(node) {
        let previousNode = node.prev, nextNode = node.next;
        if (previousNode) {
